...
## Telegram Bildirimleri (Yeni Teklif Talebi)

Yeni bir teklif formu gönderildiğinde, Telegram botunuz üzerinden anlık bildirim alabilirsiniz.

### a) Chat ID'nizi bulun
1. Telegram'da botunuzu açın (`t.me/eslmakine_bot`) ve bota herhangi bir mesaj gönderin (örn. "merhaba").
2. Tarayıcınızda şu adresi açın (TOKEN kısmını BotFather'ın verdiği token ile değiştirin):
   `https://api.telegram.org/botTOKEN/getUpdates`
3. Açılan JSON içinde `"chat":{"id":123456789, ...}` şeklinde bir alan göreceksiniz — o sayı sizin **chat ID**'nizdir.
4. Botu bir gruba eklediyseniz aynı yöntemle grubun (negatif sayılı) chat ID'sini bulabilirsiniz.

### b) Vercel'e ortam değişkeni ekleyin
**Settings → Environment Variables** kısmına şunları ekleyin:

| Değişken adı | Değer |
|---|---|
| `TELEGRAM_BOT_TOKEN` | BotFather'ın verdiği token (`8607793850:AAGU...`) |
| `TELEGRAM_CHAT_ID` | 2. adımda bulduğunuz chat ID. Birden fazla kişiye/gruba göndermek isterseniz virgülle ayırın: `111111,222222` |

Ekledikten sonra projeyi yeniden deploy edin. Bundan sonra her yeni teklif talebinde botunuzdan otomatik mesaj gelecektir.
